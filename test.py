...
def test(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if save_json:
                ...
                image_id = int(path.stem.split("_")[0])
    ...
    if save_json and len(jdict):
        ...
        anno_json = "/staging/dataset_donation/round_2/coco-style/zod_full_original_val.json"  # annotations json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            for i, cat_ids in enumerate([[1], [2], [3], [1, 2, 3]]):
                anno = COCO(anno_json)  # init annotations api
                pred = anno.loadRes(pred_json)  # init predictions api
                if i == 0:
                    name = "Vehicle"
                elif i == 1:
                    name = "VulnerableVehicle"
                elif i == 2:
                    name = "Pedestrian"
                else:
                    name = "All"  # update results (mAP@0.5:0.95, mAP@0.5)
                ...
                pred_img_ids = list(set([int(x["image_id"]) for x in jdict]))
                eval.params.imgIds = pred_img_ids  # image IDs to evaluate
                eval.params.catIds = cat_ids  # category IDs to evaluate
                ...
                output_file = save_dir / f"{name}_coco_results.json"
                with open(output_file, "w") as f:
                    json.dump(eval.stats.tolist(), f, indent=4)
