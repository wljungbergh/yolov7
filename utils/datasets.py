...
def img2label_paths_zod(img_paths: str, dataroot: str):
    # get the basename
    basenames = [os.path.basename(i) for i in img_paths]
    # get the label paths
    label_paths = [
        os.path.join(dataroot, "labels", i.replace(".jpg", ".txt")) for i in basenames
    ]
    return label_paths
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        # Check cache
        self.label_files = img2label_paths_zod(
            self.img_files, os.path.dirname(self.path)
        )  # labels
        cache_path = (
        ...
        self.label_files = img2label_paths_zod(
            cache.keys(), os.path.dirname(self.path)
        )  # update
        if single_cls:
